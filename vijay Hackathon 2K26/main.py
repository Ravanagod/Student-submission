...
@app.get("/teacher")
def teacher(request: Request, db: Session = Depends(get_db)):
...
@app.get("/plagiarism/{assignment_id}")
def plagiarism_check(request: Request, assignment_id: int, db: Session = Depends(get_db)):
    ...
    for s in submissions:
        try:
            with open(s.file_path, "r", errors="ignore") as f:
                texts.append(f.read())
        except Exception:
            texts.append("")
    ...
    try:
        results = check_plagiarism(texts)
    except Exception:
        results = []
